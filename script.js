...
function takeFriendPhoto() {
  ...
  preview.innerHTML = `<img src="${friendPhotoData}" alt="Ảnh kỷ niệm" class="photobooth-media">`;
...
function exportToExcel() {
  ...
  XLSX.writeFile(
    wb,
    `thong-tin-nguoi-yeu-${new Date().toISOString().split("T")[0]}.xlsx`
  );
}
let stream = null;
...
function showFriendPhotoSuccess() {
  ...
  notification.style.cssText = `
    ...
    left: 50%;
    transform: translate(-50%, -50%);
    background: linear-gradient(135deg, rgba(255, 192, 203, 0.9), rgba(255, 140, 0, 0.9));
    color: white;
    padding: 20px 30px;
    border-radius: 15px;
    font-size: 18px;
...
function downloadFriendPhoto(dataUrl) {
  const link = document.createElement("a");
  link.download = `ban-be-ky-niem-${new Date().toISOString().split("T")[0]}.png`;
  link.href = dataUrl;
  link.click();
}
